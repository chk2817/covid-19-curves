...
app = dash.Dash(__name__,
                ...
                meta_tags=[{"name": "viewport", "content": "width=device-width, initial-scale=1"},
                           ...
                           {'property': 'og:image:secure_url',
                            'content':'https://drive.google.com/file/d/1fmnHYz5DdhKmZ9nOpMDtL_fuGAMJC3aY/view?usp=sharing'},
    ...
    dbc.Card([
        ...
        dbc.CardBody([
            ...
            dbc.Row([
                ...
                dbc.Col([
                    dbc.Button('Refresh',color='primary',id='timeline_refresh',className='float-right', style={'width':'100px'})
                ],className='col-12 col-md-6 col-xl-6 mb-4')
            ], className='mb-4'),
            ...
            html.Div([
                dbc.Row([
                    dbc.Col([dbc.Spinner(dcc.Graph(id='timeline_1',config=config, style={'height':'50vh'}),
                                         color='secondary', size='lg')],className='col-12 col-xl-6'),
                    dbc.Col([dbc.Spinner(dcc.Graph(id='timeline_2',config=config, style={'height':'50vh'}),
                                         color='secondary', size='lg')],className='col-12 col-md-6 col-xl-3'),
                    dbc.Col([dbc.Spinner(dcc.Graph(id='timeline_3',config=config, style={'height':'50vh'}),
                                         color='secondary', size='lg')],className='col-12 col-md-6 col-xl-3'),
                ...
                dbc.Row([
                    dbc.Col(
                        dbc.Toast(
                            ...
                            duration=3000,
                            icon="danger",
                            style={"position": "fixed", "top": 100, "right": 40, "width": 300,'zIndex':999},
                        )
                    ,className='col-12 col-md-6 col-xl-3')
                ])
            ...
            dbc.Row([
                dbc.Col([dbc.Spinner(dcc.Graph(id='detail_1',config=config, style={'height':'65vh'}),
                                     color='secondary', size='lg')],className='col-12 col-xl-6'),
                dbc.Col([dbc.Spinner(dcc.Graph(id='detail_2',config=config, style={'height':'65vh'}),
                                     color='secondary', size='lg')],className='col-12 col-md-6 col-xl-3'),
                dbc.Col([dbc.Spinner(dcc.Graph(id='detail_3',config=config, style={'height':'65vh'}),
                                     color='secondary', size='lg')],className='col-12 col-md-6 col-xl-3'),
            ],id='current_row',className='mt-4'),
        ]),
...
def update_timeline_view(sel_tab):
    if sel_tab=='tab_timeline':
        ...
        return 'mt-4 d-none', 'mt-4 d-none'
...
@app.callback([Output('timeline_1','figure'),
              ...
              [Input('timeline_refresh','n_clicks')],
...
def update_timeline_plots(n,region,subregion,country,area,split,top_limit):
    ...
    if split:
        dff_agg = dff.loc[:, [split, 'date', 'confirmed_cases', 'deaths', 'recovered', 'active', 'population']].groupby(
            ['date',split]).sum().reset_index()

        # compute the category orders
        dff_cat_order=dff_agg.loc[dff_agg['date']==end_date,[split,'confirmed_cases','population']].groupby([split]).sum()\
        ...
        dff_agg=dff.loc[:,['date','confirmed_cases','deaths','recovered','active','population']].groupby(['date']).sum().reset_index()
    ...
    fig = px.bar(dff_agg, x='date', y='confirmed_cases', color=split,
    ...
    fig2 = px.bar(dff_agg, x='date', y='deaths', color=split,
                 color_discrete_sequence=px.colors.qualitative.Dark24,
    ...
    fig3 = px.bar(dff_agg, x='date', y='recovered', color=split,
                  color_discrete_sequence=px.colors.qualitative.Dark24,
    ...
    if split is None:
        return fig, fig2, fig3, dash.no_update, dash.no_update, dash.no_update
...
def update_map(map_data,per_capita, sel_day, small_pop):
    ...
    cases_label = ["Cases : " + human_format(total_cases_sel_date),
                   dbc.Badge(str('{:+.0%}'.format(cases_variance)), className="ml-1", id='badge_case',
                             style={'backgroundColor': '#508caf'}),
    ...
    lbl_deaths=["Deaths : "+human_format(total_deaths_sel_date),
                dbc.Badge(str('{:+.0%}'.format(death_variance)),className="ml-1", id='badge_death',
                          style={'backgroundColor': '#508caf'}),
            ...
            dbc.Row([
                dbc.Col([
                    ...
                    dcc.Dropdown(options=subregion_options, id='prog_dd_subregion', value=[],
                                 multi=True, style={'fontSize': '100%'}, placeholder='Select Sub-Region(s)', className='mb-4'),
                    dcc.Dropdown(options=country_options, id='prog_dd_country',value=['Italy','Spain','France','Switzerland'],
                                 multi=True,style={'fontSize':'100%'}, placeholder='Select Country(ies)', className='mb-4'),
                    dcc.Dropdown(options=area_options, id='prog_dd_area',value=['China - Hubei','US - New York'],
                                 multi=True,style={'fontSize':'100%'}, placeholder='Select Area(s)', className='mb-4'),
                    dbc.Row([
                    ...
                    dbc.Checklist(options=[{"label": "Log-Scale", "value": True}],value=[True],id="prog_logscale", switch=True, className='mb-4'),
                    dbc.Checklist(options=[{"label": "Show Development Time", "value": True, 'disabled': False}],value=[True],
                          id="prog_dd_devtime", switch=True, className='mb-4'),
                    dbc.Button('Refresh',color='primary',id='prog_refresh',className='float-right', style={'width':'100px'})
                ],className='col-12 col-md-6 col-xl-3'),
                ...
                dbc.Col([
                    ...
                    html.Div(
                        dbc.Spinner(
                            dcc.Graph(id='prog_1',config=config, style={'height':'70vh'}),color='secondary', size='lg'),
                    id='div_1'),
                    html.Div(
                ...
                dbc.Col([
                    ...
                    html.Div([
                        ...
                        html.Span('Plotly',className='text-uppercase',
                                  style={'fontSize': '1.25rem','fontWeight':'700','fontFamily':'campaign,sans-serif'}),
                    ]),
                ])
